...
MCP 注册表（`mcps/registry.json`），使用自建的统一格式登记所有 MCP 服务。每条记录包含名称、传输协议（stdio 或 streamable_http）、连接参数和说明文字。注册表同时给人读（`registry.md`）和给程序执行。API Key、Token 等直接写在配置中，配置即所见。
...
| `lib/caplib.py` | 公共库：JSON 读写、配置加载、格式校验、session 管理 |
...
1. 解压 release 包
2. 复制 `config.example.json` → `config.local.json`，填入 DeepSeek API Key
3. MCP 的 Key 直接写在 `mcps/registry.json` 各配置的 params 里

首次使用：复制 `docs/AI引导提示词.md` 的全部内容发给你的 AI，AI 会自动完成初始化（了解机制、迁移现有 MCP/skill 入库）。

### 日常命令速查

```powershell
python tools/advisor/ability_suggest.py "需求描述"          # 咨询该用什么
